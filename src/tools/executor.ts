...
import { parseToolCallBlock } from '../routes/stream-parser.ts';
...
export function parseToolCallsFromContent(content: string): {
  ...
  while (true) {
    ...
    try {
      const parsed = parseToolCallBlock(block, TOOL_START_TAG, []);
      toolCalls.push({
        id: 'call_' + uuidv4(),
        name: parsed.name || '',
      ...
      textContent += TOOL_START_TAG + block + TOOL_END_TAG;
    }

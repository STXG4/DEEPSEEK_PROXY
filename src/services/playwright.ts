...
// ─── PoW Header Cache ──────────────────────────────────────────────────────────
const POW_CACHE_TTL_MS = 30_000; // 30 seconds
let cachedHeaders: { headers: Record<string, string>; chatSessionId: string; parentMessageId: number | null; timestamp: number } | null = null;
...
export async function getDeepSeekHeaders(forceNew = false): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: number | null }> {
  ...
  if (!forceNew && cachedHeaders && (Date.now() - cachedHeaders.timestamp < POW_CACHE_TTL_MS)) {
    return { headers: cachedHeaders.headers, chatSessionId: cachedHeaders.chatSessionId, parentMessageId: cachedHeaders.parentMessageId };
  }
